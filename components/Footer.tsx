...
export function Footer() {
  ...
  const hasAbout = Boolean(data.currentActivity?.description || data.currentActivity?.name || data.currentActivity?.theme);
  ...
  const navLinks = [
    ...
    { label: 'Sobre a Atividade', href: '#sobre', show: hasAbout },
    { label: 'Momentos em Destaque', href: '#destaques', show: hasHighlights },
    { label: 'Galeria de Fotos', href: '#fotos', show: hasPhotos },
    { label: 'Galeria de Vídeos', href: '#videos', show: hasVideos },
    { label: 'Agenda de Atividades', href: '#atividades', show: hasEvents },
    { label: 'Redes Sociais', href: '#redes-sociais', show: hasSocial },
    ...
    <footer id="contacto" className="bg-[#141414] text-neutral-300 border-t border-neutral-800 relative overflow-hidden pt-16 pb-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-8 pb-12 border-b border-neutral-800/80">
          ...
          <div className="lg:col-span-4 space-y-4">
            {data.churchName ? (
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-sm bg-[#1A1A1A] border border-neutral-700 flex items-center justify-center text-[#C5A059]">
          ...
          {/* Sede & Contactos (apenas se preenchidos) */}
          {hasContact && (
            <div className="lg:col-span-5 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs text-neutral-300 font-light">
                {(data.address || data.cityCountry) && (
                  <div className="flex items-start gap-2.5">
                    <MapPin className="w-3.5 h-3.5 text-[#C5A059] flex-shrink-0 mt-0.5" />
                    <span>{[data.address, data.cityCountry].filter(Boolean).join(', ')}</span>
                  </div>
                )}
          ...
          {/* Redes Sociais (apenas se preenchidas) */}
          {hasSocial && (
        ...
        {/* Bottom Bar with Admin Link and Baobá Universe Attribution */}
